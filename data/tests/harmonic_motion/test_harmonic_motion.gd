...
func _process(delta):
	$Sprite.position = harmonic_motion.calculate_v2( $Sprite.position, $target_sprite.position )
...
func _on_Timer_timeout():
	...
	$target_sprite.position = Vector2(rand_range(v_size.x * 0.15, v_size.x - (v_size.x * 0.15) ),
									rand_range(v_size.y * 0.15, v_size.y - (v_size.y * 0.15) )
									)
